...
int main(int argc, char **argv)
{
    ...
    while (bag.has_next())
    {
        ...
        if (msg->topic_name == "/lidar_odometry/deskewed_scan_points") {
            RCLCPP_INFO(rclcpp::get_logger("MolaScan"), "Received message on topic: /lidar_odometry/deskewed_scan_points");
            ...
            if (!cloud_msg || cloud_msg->data.empty()) {
                RCLCPP_DEBUG(rclcpp::get_logger("MolaScan"), "Empty PointCloud2 message on /lidar_odometry/deskewed_scan_points");
                continue;
            }

            size_t num_points = cloud_msg->width * cloud_msg->height;

            RCLCPP_INFO(rclcpp::get_logger("MolaScan"), "Processing %zu points from /lidar_odometry/deskewed_scan_points", num_points);
            ...
            RCLCPP_INFO(rclcpp::get_logger("MolaScan"), "Added %zu points (total: %zu)", num_points, points_global.size());
        }

        if (msg->topic_name == "/lidar_odometry/pose") {
            RCLCPP_INFO(rclcpp::get_logger("MolaOdometry"), "Received message on topic: /lidar_odometry/pose");
            ...
            if (!odom_msg) {
                RCLCPP_ERROR(rclcpp::get_logger("MolaOdometry"), "Odometry message deserialization error!");
                return 1;
            }
            ...
            RCLCPP_INFO(rclcpp::get_logger("MolaOdometry"), "Added position to trajectory: x=%.3f, y=%.3f, z=%.3f", x, y, z);
        }
    }
