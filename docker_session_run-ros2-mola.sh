...
echo "=== MOLA ROS2 rosbag pipeline ==="
...
echo "[record] start"
ros2 bag record /lidar_odometry/deskewed_scan_points /lidar_odometry/pose \
...
docker run -it --rm \
  ...
  /bin/bash -c "
    source /opt/ros/humble/setup.bash
    source /ros2_ws/install/setup.bash
    ros2 run mola-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-mola\"
  "
